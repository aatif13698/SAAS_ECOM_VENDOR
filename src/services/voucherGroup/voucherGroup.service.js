...
const getFinancialYears = async (page, keyWord, perPage) => {
...
const create = async (data) => {
    ...
    return await axios.post(`${import.meta.env.VITE_BASE_URL}/api/vendor/accounts/vg/create/voucherGroup`, data, {
        headers: {
            Authorization: `Bearer ${authToken}`,
        }
...
const getList = async (page, keyWord, perPage, currentLevel, levelId) => {
    ...
    try {
        const response = await axios.get(
            `${import.meta.env.VITE_BASE_URL}/api/vendor/accounts/vg/list/voucherGroup?keyword=${keyWord}&perPage=${perPage}&page=${page}&clientId=${clientId}&level=${currentLevel}&levelId=${levelId}`,
            {
                headers: {
                    Authorization: `Bearer ${authToken}`,
                }
...
const getAll = async (currentLevel, levelId) => {
    ...
    try {
        const response = await axios.get(
            `${import.meta.env.VITE_BASE_URL}/api/vendor/accounts/vg/all/voucherGroup?clientId=${clientId}&level=${currentLevel}&levelId=${levelId}`,
            {
                headers: {
                    Authorization: `Bearer ${authToken}`,
                }
...
const update = async (data) => {
    const authToken = localStorage.getItem("saas_client_token");
    const clientId = localStorage.getItem("saas_client_clientId");
    return await axios.put(`${import.meta.env.VITE_BASE_URL}/api/vendor/accounts/vg/update/voucherGroup`, data, {
        headers: {
            Authorization: `Bearer ${authToken}`,
        }
...
const activeInactive = async (data) => {
    const authToken = await localStorage.getItem("saas_client_token");
    return await axios.post(`${import.meta.env.VITE_BASE_URL}/api/vendor/accounts/vg/activeInactive/voucherGroup`, data, {
        headers: {
            Authorization: `Bearer ${authToken}`,
        }
    });
