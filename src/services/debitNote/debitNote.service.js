...
const create = async (data) => {
    ...
    return await axios.post(`${import.meta.env.VITE_BASE_URL}/api/vendor/purhcase/dn/create/debitNote`, data, {
        headers: {
...
const getList = async (page, keyWord, perPage, currentLevel, levelId) => {
    ...
    try {
        const response = await axios.get(
            `${import.meta.env.VITE_BASE_URL}/api/vendor/purhcase/dn/list/debitNote?keyword=${keyWord}&perPage=${perPage}&page=${page}&clientId=${clientId}&level=${currentLevel}&levelId=${levelId}`,
            {
                headers: {
                    Authorization: `Bearer ${authToken}`,
                }
...
const getParticular = async (id) => {
    ...
    try {
        const response = await axios.get(
            `${import.meta.env.VITE_BASE_URL}/api/vendor/purhcase/dn/get/${clientId}/${id}`,
            {
                headers: {
                    Authorization: `Bearer ${authToken}`,
                }
