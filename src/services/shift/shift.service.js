...
const create = async (data) => {
    ...
    return await axios.post(`${import.meta.env.VITE_BASE_URL}/api/vendor/shift/create/shift`, data, {
        headers: {
...
const getList = async (page, keyWord, perPage, currentLevel, levelId) => {
    ...
    try {
        const response = await axios.get(
            `${import.meta.env.VITE_BASE_URL}/api/vendor/shift/list/shift?keyword=${keyWord}&perPage=${perPage}&page=${page}&clientId=${clientId}&level=${currentLevel}&levelId=${levelId}`,
            {
                headers: {
                    Authorization: `Bearer ${authToken}`,
                }
