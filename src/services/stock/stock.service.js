...
const getStockListOfCustomerBySaleId = async ( supplier , id) => {
    const authToken = localStorage.getItem("saas_client_token");
    const clientId = localStorage.getItem("saas_client_clientId");

    const response = await axios.get(`${import.meta.env.VITE_BASE_URL}/api/vendor/stock/listStock/all/of/customer/by/saleInv?customerId=${supplier}&&saleInvId=${id}&&clientId=${clientId}`, {
        headers: {
            Authorization: `Bearer ${authToken}`,
        }
...
export default {
    ...
    getStockListOfCustomerBySaleId,
