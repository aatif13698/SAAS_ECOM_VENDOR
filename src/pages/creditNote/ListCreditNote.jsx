...
const ListCreditNote = ({ noFade, scrollContent }) => {
    ...
    const columns = [
        ...
        {
            name: "Date",
            selector: (row) => formatDate(row?.cnDate),
            sortable: true,
            style: {
                width: "20px", // Set the desired width here
            },
        ...
        {
            name: "Amount",
            selector: (row) => Number(row.totalOrderAmount).toFixed(2) ,
            sortable: true,
            style: {
                width: "20px", // Set the desired width here
            },
    ...
    const debounceSearch = useCallback(
        debounceFunction(
            async (nextValue) => {
                try {
                    const response = await creditNoteService.getList(page, nextValue, perPage, currentLevel, levelId);
                    setTotalRows(response?.data?.count);
                    setPaginationData(response?.data?.creditNotes);
                } catch (error) {
    ...
    async function getList() {
        try {
            const response = await creditNoteService.getList(page, keyWord, perPage, currentLevel, levelId);
            setTotalRows(response?.data?.count);
            setPaginationData(response?.data?.creditNotes);
            setPending(false);
        } catch (error) {
