...
const AssetRequests = ({ noFade }) => {
    ...
    const handleActive = async (row) => {
        ...
        try {
            ...
            const response = await assetRequestService.activeInactive(dataObject);
    ...
    const debounceSearch = useCallback(
        debounceFunction(
            async (nextValue) => {
                try {
                    const response = await assetRequestService.getList(page, nextValue, perPage, currentLevel, levelId);
    ...
    async function getList() {
        try {
            const response = await  assetRequestService.getList(page, keyWord, perPage, currentLevel, levelId);
            setTotalRows(response?.data?.count);
            setPaginationData(response?.data?.assetRequests);
            setPending(false);
        } catch (error) {
    ...
    const handlePageChange = async (page) => {
        try {
            const response = await assetRequestService.getList(page, keyWord, perPage);
            setTotalRows(response?.data?.count);
            setPaginationData(response?.data?.assetRequests);
            setPage(page);
        } catch (error) {
