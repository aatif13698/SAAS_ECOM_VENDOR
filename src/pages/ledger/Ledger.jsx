...
const Ledger = ({ noFade, scrollContent }) => {
  ...
  const handleView = (row) => {
    ...
    setUserId(id);
    setIsViewed(true);
    navigate("/create-ledger", { state: { id, row, name } });
  };
  ...
  const columns = [
    ...
    {
      name: "Under Group",
      selector: (row) => row?.ledgerGroupId?.groupName,
      sortable: true,
      style: {
        width: "20px", // Set the desired width here
      },
  ...
  const subHeaderComponent = (
    <div className="w-full grid xl:grid-cols-2 md:grid-cols-1 md:text-start gap-3  items-center">
      <div className="table-heading text-start ">Ledger List</div>
