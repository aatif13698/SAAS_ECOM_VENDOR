...
const RequestShifts = ({ noFade, scrollContent }) => {
    ...
    const handleActive = async (row) => {
        ...
        try {
            ...
            const response = await requestShiftService.activeInactive(dataObject);
    ...
    async function getList() {
        try {
            const response = await requestShiftService.getList(page, keyWord, perPage, currentLevel, levelId);
            setTotalRows(response?.data?.count);
            setPaginationData(response?.data?.changeShifts);
            setPending(false);
        } catch (error) {
            setPending(false);
    ...
    const subHeaderComponent = (
        <div className="w-full grid xl:grid-cols-2 md:grid-cols-1 md:text-start gap-3  items-center">
            <div className="table-heading text-start ">Change Shifts List</div>
