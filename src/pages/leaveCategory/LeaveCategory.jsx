...
const LeaveCategory = ({ noFade, scrollContent }) => {
    ...
    const handleActive = async (row) => {
        ...
        try {
            ...
            setTotalRows(response?.data?.data?.count);
            setPaginationData(response?.data?.data?.leaveCategories);
    ...
    const columns = [
        ...
        },
        {
            ...
            selector: (row) => row.carryOverLimit,
            sortable: true,
            style: {
                width: "20px", // Set the desired width here
            },
    ...
    const debounceSearch = useCallback(
        debounceFunction(
            async (nextValue) => {
                try {
                    const response = await leaveCategoryService.getList(page, nextValue, perPage, currentLevel, levelId);
                    setTotalRows(response?.data?.count);
                    setPaginationData(response?.data?.leaveCategories);
                } catch (error) {
    ...
    async function getList() {
        try {
            const response = await leaveCategoryService.getList(page, keyWord, perPage, currentLevel, levelId);
            setTotalRows(response?.data?.count);
            setPaginationData(response?.data?.leaveCategories);
            setPending(false);
        } catch (error) {
    ...
    const handlePageChange = async (page) => {
        try {
            const response = await leaveCategoryService.getList(page, keyWord, perPage);
            setTotalRows(response?.data?.count);
            setPaginationData(response?.data?.leaveCategories);
            setPage(page);
        } catch (error) {
    ...
    const subHeaderComponent = (
        <div className="w-full grid xl:grid-cols-2 md:grid-cols-1 md:text-start gap-3  items-center">
            <button className={`w-fit bg-lightBtn hover:bg-lightBtnHover dark:bg-darkBtn hover:dark:bg-darkBtnHover text-white dark:hover:text-black-900  px-3 py-2 rounded   `}
                onClick={() => navigate("/create-leave-category")}
            >
