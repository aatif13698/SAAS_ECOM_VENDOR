...
const VoucherGroup = ({ noFade, scrollContent }) => {
    ...
    const handleActive = async (row) => {
        ...
        try {
            ...
            const response = await voucherGroupService.activeInactive(dataObject);
            setTotalRows(response?.data?.data?.count);
            setPaginationData(response?.data?.data?.voucherGroups);
    ...
    const subHeaderComponent = (
        <div className="w-full grid xl:grid-cols-2 md:grid-cols-1 md:text-start gap-3  items-center">
            <div className="table-heading text-start ">Voucher Group List</div>
