...
const Holiday = ({ noFade, scrollContent }) => {
    ...
    const handleView = (row) => {
        ...
        setUserId(id);
        setIsViewed(true);
        navigate("/create-holiday", { state: { id, row, name } });
    };
    ...
    const handleActive = async (row) => {
        ...
        try {
            ...
            const response = await holidayService.activeInactive(dataObject);

            setTotalRows(response?.data?.data?.count);
            setPaginationData(response?.data?.data?.holidays);
    ...
    const debounceSearch = useCallback(
        debounceFunction(
            async (nextValue) => {
                try {
                    const response = await holidayService.getList(page, nextValue, perPage, currentLevel, levelId);
                    setTotalRows(response?.data?.count);
                    setPaginationData(response?.data?.holidays);
                } catch (error) {
    ...
    async function getList() {
        try {
            const response = await holidayService.getList(page, keyWord, perPage, currentLevel, levelId);
            setTotalRows(response?.data?.count);
            setPaginationData(response?.data?.holidays);
            setPending(false);
        } catch (error) {
    ...
    const handlePageChange = async (page) => {
        try {
            const response = await holidayService.getList(page, keyWord, perPage);
            setTotalRows(response?.data?.count);
            setPaginationData(response?.data?.holidays);
            setPage(page);
        } catch (error) {
