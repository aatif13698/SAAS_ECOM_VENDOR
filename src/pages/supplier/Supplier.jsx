...
const Supplier = ({ noFade, scrollContent }) => {
    ...
    async function getList() {
        try {
            const response = await supplierService.getAllList(page, keyWord, perPage);
            setTotalRows(response?.data?.count);
            setPaginationData(response?.data?.suppliers);
            setPending(false);
        } catch (error) {
    ...
    const subHeaderComponent = (
        <div className="w-full grid xl:grid-cols-2 md:grid-cols-1 md:text-start gap-3  items-center">
            <div className="table-heading text-start ">Supplier List</div>
