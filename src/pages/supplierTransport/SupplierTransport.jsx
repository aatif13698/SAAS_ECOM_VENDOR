...
const SupplierTransport = ({ noFade, scrollContent }) => {
    ...
    const handleView = (row) => {
        ...
        setUserId(id);
        setIsViewed(true);
        navigate("/create-transport", { state: { id, row } });
    };
    ...
    const handleActive = async (row) => {
        ...
        try {
            ...
            const response = await supplierService.activeInActiveTransport(dataObject);

            setTotalRows(response?.data?.data?.count);
            setPaginationData(response?.data?.data?.transports);
    ...
    const columns = [
        ...
        {
            name: "Email",
            selector: (row) => row.transporterEmail,
            sortable: true,
            style: {
                width: "20px", // Set the desired width here
            },
        ...
        {
            name: "Phone",
            selector: (row) => row.transporterPhone,
            sortable: true,
            style: {
                width: "20px", // Set the desired width here
            },
    ...
    async function getList() {
        try {
            const response = await supplierService.getAllTransportersList(page, keyWord, perPage);
            setTotalRows(response?.data?.count);
            setPaginationData(response?.data?.transports);
            setPending(false);
        } catch (error) {
            setPending(false);
            console.log("error while fetching lisr");
        }
    }
    ...
    const subHeaderComponent = (
        <div className="w-full grid xl:grid-cols-2 md:grid-cols-1 md:text-start gap-3  items-center">
            <div className="table-heading text-start ">Transporter List</div>
