...
const BusinessUnit = ({ noFade, scrollContent }) => {
    ...
    const handleActive = async (row) => {
        ...
        try {
            ...
            console.log("response active", response?.data?.data?.businessUnits);
    ...
    const subHeaderComponent = (
        <div className="w-full grid xl:grid-cols-2 md:grid-cols-1 md:text-start gap-3  items-center">
            <div className="table-heading text-start ">Business Unit List</div>
...
//                         <div key={row._id} className={`p-6 rounded-lg shadow-md ${isDark ? 'bg-gray-800 text-white' : 'bg-white text-gray-900'}`}>
