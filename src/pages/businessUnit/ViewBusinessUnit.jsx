    ...
    return (
        <div className={`min-h-screen pb-8 relative ${isDark ? 'bg-gray-900 text-white' : 'bg-gray-100 text-gray-900'}`}>
            ...
            <div className="bg-gray-200">
                ...
                {/* Tab Content */}
                {tabValue === 0 && <Overview data={businessData} isDark={isDark} branches={branches} setBranches={setBranches} />}
...
const Overview = ({ data, isDark, branches, setBranches }) => {
    const navigate = useNavigate();
    ...
    const handleDelete = async (row) => {
        if (window.confirm(`Are you sure you want to delete the branch "${row.name}"?`)) {
            try {
                // Assuming employeeService has a deleteBranch method; adjust as per actual service
                await employeeService.deleteBranch(row._id); // Placeholder; implement actual API if available
                setBranches(branches.filter(b => b._id !== row._id));
                if (selectedBranch?._id === row._id) {
        ...
        <div className="md:p-6 p-2">
            ...
            <div className={`rounded-lg shadow-md md:p-6 p-2 mb-6 ${isDark ? 'bg-gray-800' : 'bg-white'}`}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-6">
                    {branches && branches?.length > 0 ? branches?.map((row) => (
...
const BranchOverview = ({ data, isDark }) => {
    return (
        <div className="md:p-1 p-2">
            <div className={`rounded-lg md:p-6 p-2 mb-6 ${isDark ? 'bg-gray-800' : 'bg-white'}`}>
                <div className={`rounded-lg p-6 mb-6 ${isDark ? 'bg-gray-800' : 'bg-gray-100'}`}>
                    <div className="flex items-center gap-4 mb-4">
                        <div className="relative">
                            <img
