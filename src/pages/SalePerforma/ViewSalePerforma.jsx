...
// import salePerformaService from '@/services/purchaseInvoice/purchaseInvoice.service';
...
function ViewSalePerforma() {
    ...
    useEffect(() => {
        ...
        async function getInvoice(id) {
            try {
                setLoading(true)
                const response = await salePerformaService.getParticular(id);
    ...
    return (
        <div className="relative min-h-screen pb-8 bg-gray-300 rounded-md dark:bg-gray-900">
            <div className="sticky top-14 z-10 bg-white dark:bg-gray-800 p-4 flex justify-between items-center shadow-md">
                <div className="flex items-center gap-3 cursor-pointer hover:text-blue-500" onClick={() => navigate("/performa-invoice-list")}>
                    <BiArrowBack className="text-xl" />
                    <h3 className="md:text-lg text-sm font-semibold text-gray-800 dark:text-gray-200">
                        Performa # {poData?.spNumber || 'N/A'}
                    </h3>
                </div>
            ...
            >
                ...
                <div className="text-center mb-8" style={{ lineHeight: 1.2 }}>
                    <h1 style={{ fontSize: '26pt', margin: 0, color: '#1a1a1a' }}>PERFORMA</h1>
                    <p style={{ margin: '8px 0', fontSize: '12pt', color: '#444' }}>
                        <strong>SP Number:</strong> {poData.spNumber || 'N/A'} &nbsp;|&nbsp; <strong>Date:</strong> {poData.spDate ? formatDate(poData.spDate) : 'N/A'}
                    </p>
                </div>
...
export default ViewSalePerforma;