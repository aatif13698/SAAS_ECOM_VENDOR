...
import { removeItemsList, resetQuotation, setAccountNumber, setBankName, setBranch, setBranchName, setBusinessUnit, setIfscCode, setIsInterState, setItemsList, setLevel, setNotes, setPaidAmount, setspDate, setspNumber, setShippingAddress, setSupplier, setWarehouse } from '../../store/slices/performa/performaSlice';
...
const CreatePerforma = ({ noFade, scrollContent }) => {
  ...
  useEffect(() => {
    ...
    setFormData((prev) => ({
      ...
      spDate: purhcaseOrderDraftData?.spDate,
  ...
  const handleInputChange = (e, section) => {
    ...
    if (section) {
      ...
      if (name == "spNumber") {
        dispatch(setspNumber(value))
      }
      if (name == "spDate") {
        dispatch(setspDate(value))
      }
      if (name === "notes") {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const dataObject = {
        ...
        spNumber: formData?.spNumber,
        spDate: formData?.spDate,
  ...
  function resetAllAndNavigate() {
    ...
    navigate('/performa-invoice-list');
  ...
  return (
    <div className=' bg-white dark:bg-darkSecondary rounded-lg relative'>
      <div className='bg-white shadow-sm flex gap-2 justify-between dark:bg-darkSecondary sticky z-[99] top-[3.54rem] p-4  border-b-2'>
        <div>
          <h2 className="text-xl font-semibold   text-gray-700">Create Performa</h2>
        </div>
