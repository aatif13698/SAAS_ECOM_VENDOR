...
const SalePerformaList = ({ noFade, scrollContent }) => {
    ...
    const handleView = (row) => {
        ...
        const name = "view"
        setIsViewed(true);
        navigate(`/view/performa-invoice/${encryptId(row._id)}`, { state: { id: row._id, name: "view" } });
    ...
    const columns = [
        {
            name: "Date",
            selector: (row) => formatDate(row?.spDate),
            sortable: true,
            style: {
    ...
    const debounceSearch = useCallback(
        debounceFunction(
            async (nextValue) => {
                try {
                    const response = await salePerformaService.getList(page, nextValue, perPage, currentLevel, levelId);
                    setTotalRows(response?.data?.count);
                    setPaginationData(response?.data?.performas);
                } catch (error) {
    ...
    async function getList() {
        try {
            const response = await salePerformaService.getList(page, keyWord, perPage, currentLevel, levelId);
            setTotalRows(response?.data?.count);
            setPaginationData(response?.data?.performas);
            setPending(false);
        } catch (error) {
    ...
    function handleCreate() {
        navigate("/create-performa-invoice")
    }
...
export default SalePerformaList;
