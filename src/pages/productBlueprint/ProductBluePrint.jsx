...
const ProductBluePrint = ({ noFade, scrollContent }) => {
    ...
    const handleView = (row) => {
        ...
        setUserId(id);
        setIsViewed(true);
        navigate("/create-product", { state: { id, row, isViewed : true } });
    };
    ...
    async function deleteOne(id) {
        try {
            ...
            const response = await productBlueprintService.deleteOne(dataObject);
            setTotalRows(response?.data?.data?.count);
            setPaginationData(response?.data?.data?.roductBluePrints);
    ...
    const handleEdit = (row) => {
        ...
        setUserId(id);
        setIsViewed(false);
        navigate("/create-product", { state: { id, row, isViewed : false } });
    };
    ...
    const handleActive = async (row) => {
        ...
        try {
            ...
            const response = await productBlueprintService.activeInactive(dataObject);
    ...
    async function getList() {
        try {
            const response = await productBlueprintService.getList(page, keyWord, perPage, currentLevel, levelId);
            setTotalRows(response?.data?.count);
            setPaginationData(response?.data?.roductBluePrints);
            setPending(false);
        } catch (error) {
            setPending(false);
    ...
    const subHeaderComponent = (
        <div className="w-full grid xl:grid-cols-2 md:grid-cols-1 md:text-start gap-3  items-center">
            <div className="table-heading text-start ">Product Blueprint List</div>
