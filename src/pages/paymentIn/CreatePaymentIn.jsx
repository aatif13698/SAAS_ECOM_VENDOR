...
const CreatePaymentIn = ({ noFade, scrollContent }) => {
    ...
    console.log("paymentInDrafData", paymentInDrafData);
    ...
    useEffect(() => {
        if (!paymentInDrafData?.level) return;
        setFormData((prev) => {
            return ({
                ...prev,
                level: paymentInDrafData.level ?? prev.level,
                businessUnit: paymentInDrafData.businessUnit ? paymentInDrafData.businessUnit : prev.businessUnit,
                branch: paymentInDrafData.branch ? paymentInDrafData.branch : prev.branch,
                warehouse: paymentInDrafData.warehouse ? paymentInDrafData.warehouse : "",
                supplier: paymentInDrafData?.customer ? paymentInDrafData?.customer : null,
                paymentInNumber: paymentInDrafData?.paymentInNumber,
                paymentInDate: paymentInDrafData?.paymentInDate,
        ...
        const warehouseDetail = activeWarehouse.find((item) => item?._id === paymentInDrafData.warehouse);
        if (warehouseDetail) setCurrentWarehouseDetal(warehouseDetail);
    }, [paymentInDrafData]);
    ...
    const handleInputChange = (e) => {
        ...
        if (name === "paidAmount") {
            ...
            if (name === "paymentInNumber") dispatch(setPaymentInNumber(value));
            if (name === "paymentInDate") dispatch(setPaymentInDate(value));
    ...
    const handleAutoAllocate = (target) => {
        ...
        if (target > currentSum) {
            ...
            const nonFixed = newAllocs
                ...
                .sort((a, b) => new Date(a.siDate) - new Date(b.siDate));
            ...
            const nonFixedWithApplied = newAllocs
                ...
                .sort((a, b) => new Date(b.siDate) - new Date(a.siDate));
    ...
    // useEffect(() => {
    //     if (allocations && paymentInDrafData) {
    //         handleAutoAllocate(paymentInDrafData?.paidAmount)
    //     }
    // }, [allocations, paymentInDrafData])
    ...
    useEffect(() => {
        if (unPaidInvoices.length > 0) {
            const sorted = [...unPaidInvoices].sort((a, b) => new Date(a.siDate) - new Date(b.siDate));
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const dataObject = {
                ...
                paymentInNumber: formData.paymentInNumber,
    ...
    function resetAllAndNavigate() {
        ...
        dispatch(resetPaymentIn())
        navigate('/payment-in-list');
    }
    ...
    return (
        <div className='bg-white dark:bg-darkSecondary rounded-lg relative'>
            <div className='bg-white shadow-sm flex gap-2 justify-between dark:bg-darkSecondary sticky z-[99] top-[3.54rem] p-4 border-b-2'>
                <div>
                    <h2 className="text-xl font-semibold text-gray-700">Create Payment In</h2>
                </div>
                <div className='flex gap-2'>
                    {paymentInDrafData?.level && (
                        <button
                            type='button'
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                            ...
                            <div className="bg-white dark:bg-transparent lg:col-span-1 md:col-span-2 rounded-lg border border-gray-200">
                                <div className='bg-gray-100 dark:bg-transparent dark:border-b-[2px] dark:border-white p-2 rounded-t-lg'>
                                    <h3 className="md:text-base text-base font-medium mb-2 text-gray-700">Payment In Details</h3>
                                </div>
                                <div className="h-[80%] p-2 grid grid-cols-1 md:grid-cols-2 gap-2">
                                    ...
                                    <div>
                                        <label className="block text-sm font-medium text-gray-700 mb-1">Payment In Date</label>
                                        <input
                                            type="date"
                                            name="paymentInDate"
                                ...
                                <Dialog.Panel className={`w-full transform rounded-md text-left align-middle shadow-xl transition-all max-w-3xl ${isDark ? "bg-darkSecondary text-white" : "bg-light"}`}>
                                    ...
                                    <div className="p-4 overflow-y-auto max-h-[50vh]">
                                        {suppliers.length > 0 ? (
                                            ...
                                            <p className={isDark ? 'text-gray-300' : 'text-gray-500'}>No customer available</p>
                                        )}
                                    </div>
