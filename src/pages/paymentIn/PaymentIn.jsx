...
const PaymentIn = ({ noFade, scrollContent }) => {
    ...
    const handleView = (row) => {
        ...
        const name = "view"
        setIsViewed(true);
        navigate(`/view-payment-in/${encryptId(row._id)}`);
    };
    ...
    const columns = [
        {
            name: "Date",
            selector: (row) => formatDate(row?.paymentInDate),
            sortable: true,
            style: {
        ...
        {
            name: "Payment Number",
            selector: (row) => row?.paymentInNumber,
            sortable: true,
            style: {
                width: "20px", // Set the desired width here
            },
    ...
    const debounceSearch = useCallback(
        debounceFunction(
            async (nextValue) => {
                try {
                    const response = await salePaymentInConfigureService.getPaymentIn(page, nextValue, perPage, currentLevel, levelId);
                    setTotalRows(response?.data?.count);
                    setPaginationData(response?.data?.paymentIn);
                } catch (error) {
    ...
    async function getList() {
        try {
            const response = await salePaymentInConfigureService.getPaymentIn(page, keyWord, perPage, currentLevel, levelId);
            setTotalRows(response?.data?.count);
            setPaginationData(response?.data?.paymentIn);
            setPending(false);
        } catch (error) {
