...
function Category({ centered, noFade, scrollContent }) {
    ...
    const debounceSearch = useCallback(
        debounceFunction(
            async (nextValue) => {
                try {
                    setPending(true)
                    const response = await categoryService.getAllList({ page, keyword: nextValue, perPage })
    ...
    async function deleteOne({ id, page, keyword: keyWord, perPage }) {
        try {
            const response = await categoryService.deleteOne({ id, page, keyword: keyWord, perPage })
            toast.success(response?.data?.message);
            setRefresh((prev) => prev + 1)
        } catch (error) {
    ...
    const handleActive = async (row) => {
        ...
        try {
            const response = await categoryService.activeInActive({ id, status, page, keyword: keyWord, perPage })
            setShowLoadingModal(false)
            setRefresh((prev) => prev + 1)
        } catch (error) {
    ...
    async function getAllList(data) {
        try {
            const response = await categoryService.getAllList(data)
            setPaginationData(response?.data?.categories)
            setTotalRows(response?.data?.count)
            setPending(false)
        } catch (error) {
    ...
    const handlePerRowChange = async (perPage) => {
        try {
            const response = await categoryService.getAllList({ page, keyword: keyWord, perPage })
            setPaginationData(response?.data?.categories)
            setTotalRows(response?.data?.count)
            setPerPage(perPage)
            setPending(false)
