...
const ListDebitNote = ({ noFade, scrollContent }) => {
    ...
    const handleView = (row) => {
        ...
        const name = "view"
        setIsViewed(true);
        navigate(`/view/debit-note/${encryptId(row._id)}`, { state: { id: row._id, name: "view" } });
    ...
    const columns = [
        ...
        },
        {
            ...
            selector: (row) => {
                ...
                >
                    <span>{truncateText(unit, 12)}</span>
                </Tooltip>
            },
            sortable: true,
            style: {
        ...
        {
            name: "Date",
            selector: (row) => formatDate(row?.dnDate),
            sortable: true,
            style: {
                width: "20px", // Set the desired width here
            },
    ...
    const debounceSearch = useCallback(
        debounceFunction(
            async (nextValue) => {
                try {
                    const response = await debitNoteService.getList(page, nextValue, perPage, currentLevel, levelId);
                    setTotalRows(response?.data?.count);
                    setPaginationData(response?.data?.debitNotes);
                } catch (error) {
    ...
    async function getList() {
        try {
            const response = await debitNoteService.getList(page, keyWord, perPage, currentLevel, levelId);
            setTotalRows(response?.data?.count);
            setPaginationData(response?.data?.debitNotes);
            setPending(false);
        } catch (error) {
