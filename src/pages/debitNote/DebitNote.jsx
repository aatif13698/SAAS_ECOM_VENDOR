...
const DebitNote = ({ noFade, scrollContent }) => {
    ...
    const purhcaseOrderDraftData = useSelector((state) => state.debitNoteSlice);
    const store = useSelector((state) => state);

    console.log("store adfdsf", store);
    ...
    async function getNextSerial() {
        try {
            ...
            const response = await transactionSeriesService.getNextSerialNumber(financialYear, "debit_note");
    ...
    useEffect(() => {
        if (formData?.supplier) {
            ...
            getSupplierPurchaseInvoice(formData?.supplier?._id)
        }
    }, [formData?.supplier]);
    ...
    async function getSupplierPurchaseInvoice(id) {
        try {
            const response = await debitNoteService.getAll(id);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const dataObject = {
                ...
                dnNumber: formData?.poNumber,
                dnDate: formData?.poDate,
            ...
            const response = await debitNoteService?.create(dataObject);
    ...
    return (
        <div className=' bg-white dark:bg-darkSecondary rounded-lg relative'>
            ...
            <div className='bg-white shadow-sm flex gap-2 justify-between dark:bg-darkSecondary sticky z-[99] top-[3.54rem] p-4  border-b-2'>
                ...
                <div>
                    <h2 className="text-xl font-semibold   text-gray-700">Create Debit Note</h2>
                </div>
                        ...
                        />
                            ...
                            <div className="bg-white dark:bg-transparent lg:col-span-1 md:col-span-2 rounded-lg border border-gray-200">
                                ...
                                <div className="h-[80%] p-2 grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div>
                                        <label className="block text-sm font-medium text-gray-700 mb-1">Purchase Return No</label>
                                        <input
                                            type="text"
                                            name="poNumber"
                                    ...
                                    <div>
                                        <label className="block text-sm font-medium text-gray-700 mb-1">Purchase Return Date</label>
                                        <input
                                            type="date"
                                            name="poDate"
                                    ...
                                    <div className='relative'>
                                        <label className="block text-sm font-medium text-gray-700 mb-1">Select Purchase Inv.</label>
                                        <button
                                            type="button"
