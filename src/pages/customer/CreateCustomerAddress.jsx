...
const CreateCustomerAddress = () => {
    ...
    const handleSave = async () => {
        ...
        if (!isError) {
            ...
            if (addressId) {
                try {
                    const response = await customerService.updateAddress({ ...data, addressId: addressId, customerId: id })
                ...
                try {
                    const response = await customerService.addAddress({ ...data, customerId: id })
                    setIsSubmitting(false)
                    toast.success(response.data.message)
                    setAddressId(null);
                    setShowForm(false);
    ...
    const handleDelete = async (id) => {
        ...
        try {
            const response = await customerService.deleteAddress({ addressId: id });
            toast.success(response.data.message);
            setRefresh((prev) => prev + 1)
