...
const Customer = ({ noFade, scrollContent }) => {
    ...
    const handleView = (row) => {
        ...
        setUserId(id);
        setIsViewed(true);
        navigate("/create-customer", { state: { id, row, name } });
    };
    ...
    const handleActive = async (row) => {
        ...
        try {
            ...
            const response = await customerService.activeInactive(dataObject);

            setTotalRows(response?.data?.data?.count);
            setPaginationData(response?.data?.data?.customers);
    ...
    async function getList() {
        try {
            const response = await customerService.getList(page, keyWord, perPage, currentLevel, levelId);
            setTotalRows(response?.data?.count);
            setPaginationData(response?.data?.customers);
            setPending(false);
        } catch (error) {
            setPending(false);
    ...
    const subHeaderComponent = (
        <div className="w-full grid xl:grid-cols-2 md:grid-cols-1 md:text-start gap-3  items-center">
            <div className="table-heading text-start ">Customer List</div>
