...
// const Warehouse = ({ noFade, scrollContent }) => {
...
//         const name = "view"
//         setIsViewed(true);
//         navigate("/create-warehouse", { state: { id, row, name } });
//     };
...
//             <div className="table-heading text-start ">Warehouse List</div>
...
const Warehouse = () => {
    ...
    const handleView = (row) => {
        scrollToTop();
        navigate(`/view-warehouse/${encryptId(row._id)}`, { state: { id: row._id, name: "view" } });
    };

    const handleEdit = (row) => {
        scrollToTop();
        navigate("/create-warehouse", { state: { id: row._id, name: "edit", row } });
    };
    ...
    const debounceSearch = useCallback(
        debounceFunction(async (searchTerm) => {
            await fetchWarehouses(page, searchTerm);
        }, 1000),
        [page, perPage]
    );

    const fetchWarehouses = async (currentPage = page, search = keyWord) => {
        try {
            setPending(true);
            const response = await warehouseService.getList(currentPage, search, perPage);
